...
class MLServiceNative {
  ...
  static const int outputSize = 10; // Example: 4 for bbox, 1 for confidence, 5 for classes
  ...
  Future<bool> loadModel({bool useGpu = false}) async {
    try {
      ...
      _interpreter = await Interpreter.fromAsset('yolov11.tflite', options: interpreterOptions);
      _isModelLoaded = true;
  ...
  Uint8List _imageToByteListFloat32(CameraImage image, int inputSize) {
    ...
    var convertedBytes = Float32List(1 * inputSize * inputSize * 3); // Assuming RGB input
    ...
    for (int i = 0; i < image.planes[0].bytes.length; i++) {
      ...
      buffer[pixelIndex++] = image.planes[0].bytes[i] / 255.0; // Normalize to [0, 1]
  ...
  List<DetectedObject> runObjectDetection(CameraImage cameraImage) {
    ...
    try {
      ...
      detectedObjects.add(DetectedObject(boundingBox: const Rect.fromLTWH(100, 100, 200, 200), label: 'person', confidence: 0.9));
  ...
  List<DetectedKeypoint> runKeypointDetection(CameraImage cameraImage) {
    ...
    // This is a placeholder and needs to be adjusted to your model's actual output
    var output = List.filled(1 * 17 * 3, 0).reshape([1, 17, 3]); // Example: 17 keypoints, each with x, y, confidence
    ...
    try {
      ...
      detectedKeypoints.add(DetectedKeypoint(point: const Offset(150, 150), label: 'nose', score: 0.95));
